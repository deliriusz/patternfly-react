...
const getPathIntersectionPoint = (pathNode, line) => {
    ...
    for (let i = 0; i < numSegments; i++) {
        ...
        const intersectPoint = exports.getLinesIntersection([svgPointToPoint(pos1), svgPointToPoint(pos2)], line);
        if (intersectPoint) {
...
const getPathClosestPoint = (pathNode, reference) => {
    ...
    for (let scanLength = 0; scanLength <= pathLength; scanLength += precision) {
        ...
        const scanDistance = exports.distanceToPoint(svgPointToPoint(scan), reference);
    ...
    while (precision > 0.5) {
        ...
        const beforeDistance = exports.distanceToPoint(svgPointToPoint(before), reference);
        if (beforeLength >= 0 && beforeDistance < bestDistance) {
            ...
            const afterDistance = exports.distanceToPoint(svgPointToPoint(after), reference);
...
const getPolygonAnchorPoint = (polygonNode, reference) => {
    ...
    for (let i = 0; i < points.length; i++) {
        const intersectPoint = exports.getLinesIntersection([svgPointToPoint(points[i]), svgPointToPoint(points[i === points.length - 1 ? 0 : i + 1])], [polygonCenter, reference]);
        if (intersectPoint) {
            const intersectDistance = exports.distanceToPoint(intersectPoint, reference);
