...
const getOperation = (operation, event) => {
    ...
    return operation[getModifiers((event && event.sourceEvent) || event)] || operation[Modifiers.DEFAULT];
};
        ...
        if (node) {
            ...
            () => {
                ...
                d3.select(ownerDocument)
                    .on(createKeyHandlerId('keydown'), event => action(() => {
                    const e = event;
                    if (e.key === 'Escape') {
                ...
                .on('drag', event => action(() => {
                const { pageX, pageY } = event.sourceEvent;
                const { x, y } = event;
                if (dndManager.isDragging()) {
